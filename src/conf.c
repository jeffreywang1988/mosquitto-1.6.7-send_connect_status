...
static void config__init_reload(struct mosquitto_db *db, struct mosquitto__config *config)
{
	...
	if(config->topic_notice_online){ 
		mosquitto__free(config->topic_notice_online);
	}
	if(config->topic_notice_offline){
		 mosquitto__free(config->topic_notice_offline);
	}
	if(config->topic_notice_prefix){
		mosquitto__free(config->topic_notice_prefix);
	}
...
void config__init(struct mosquitto_db *db, struct mosquitto__config *config)
{
	...
	config->topic_notice_prefix = NULL;
}
...
#  endif
					...
					if(conf__parse_string(&token, "topic_notice_online", &config->topic_notice_online, saveptr)) return MOSQ_ERR_INVAL;
				}else if(!strcmp(token, "topic_notice_offline")){
					if(conf__parse_string(&token, "topic_notice_offline", &config->topic_notice_offline, saveptr)) return MOSQ_ERR_INVAL;
				}else if(!strcmp(token, "topic_notice_prefix")){
					if(conf__parse_string(&token, "topic_notice_prefix", &config->topic_notice_prefix, saveptr)) return MOSQ_ERR_INVAL;				
				}else{
