...
static void security__disconnect_auth(struct mosquitto_db *db, struct mosquitto *context)
{
	...
	send__connect_status(db, context, ONTICE_TYPE_OFFLINE, "connection no auth");
...
int mosquitto_security_apply_default(struct mosquitto_db *db)
{
	...
	HASH_ITER(hh_id, db->contexts_by_id, context, ctxt_tmp){
		...
		if(!allow_anonymous && !context->username){
			mosquitto__set_state(context, mosq_cs_disconnecting);
			send__connect_status(db, context, ONTICE_TYPE_OFFLINE, "Check for anonymous clients when allow_anonymous is false");
			do_disconnect(db, context, MOSQ_ERR_AUTH);
			continue;
		}
		...
		if(context->listener && context->listener->ssl_ctx && (context->listener->use_identity_as_username || context->listener->use_subject_as_username)){
			...
			if(!context->ssl){
				...
				mosquitto__set_state(context, mosq_cs_disconnecting);
				send__connect_status(db, context, ONTICE_TYPE_OFFLINE, "Check for connected clients that are no longer authorised");
				do_disconnect(db, context, MOSQ_ERR_AUTH);
				continue;
			}
...
#endif
		{
			...
			if(mosquitto_unpwd_check(db, context, context->username, context->password) != MOSQ_ERR_SUCCESS){
				mosquitto__set_state(context, mosq_cs_disconnecting);
				send__connect_status(db, context, ONTICE_TYPE_OFFLINE, "username or password is fail");
				do_disconnect(db, context, MOSQ_ERR_AUTH);
				continue;
			}
		}
		...
		if(db->config->per_listener_settings){
			if(context->listener){
				...
				if(context->state != mosq_cs_active){
					mosquitto__set_state(context, mosq_cs_disconnecting);
					send__connect_status(db, context, ONTICE_TYPE_OFFLINE, "user is not allow");
					do_disconnect(db, context, MOSQ_ERR_AUTH);
					continue;
				}
			}
