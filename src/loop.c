		...
		HASH_ITER(hh_id, db->contexts_by_id, context, ctxt_tmp){
			if(context->wsi && context->sock != INVALID_SOCKET){
				if(context->keepalive && now - context->last_msg_in > (time_t)(context->keepalive)*3/2){
					...
					send__connect_status(db, context, ONTICE_TYPE_OFFLINE, "Client has exceeded keepalive*1.5!");				
			...
			if(context->sock != INVALID_SOCKET){
				...
				if(!(context->keepalive)
					...
					if(db__message_write(db, context) == MOSQ_ERR_SUCCESS){
						...
						send__connect_status(db, context, ONTICE_TYPE_OFFLINE, "write message fail!");		
			...
			HASH_ITER(hh_id, db->contexts_by_id, context, ctxt_tmp){
				if(context->sock == INVALID_SOCKET && context->session_expiry_interval > 0 && context->session_expiry_interval != UINT32_MAX){
					...
					if(now > context->session_expiry_time){
						...
						send__connect_status(db, context, ONTICE_TYPE_OFFLINE, "Expiring persistent client due to timeout");
		...
		if(pollfds[context->pollfd_index].revents & POLLOUT){
			...
			if(context->state == mosq_cs_connect_pending){
				...
				if(!getsockopt(context->sock, SOL_SOCKET, SO_ERROR, (char *)&err, &len)){
					...
					send__connect_status(db, context, ONTICE_TYPE_OFFLINE, "get socket option fail!");	
					do_disconnect(db, context, MOSQ_ERR_CONN_LOST);
					continue;
				}
			}
			...
			if(rc){
				send__connect_status(db, context, ONTICE_TYPE_OFFLINE, "write packet fail!");
	...
	HASH_ITER(hh_sock, db->contexts_by_sock, context, ctxt_tmp){
		...
		if(pollfds[context->pollfd_index].revents & POLLIN){
			...
			do{
				...
				if(rc){
					send__connect_status(db, context, ONTICE_TYPE_OFFLINE, "read packet fail!");
					do_disconnect(db, context, rc);
					continue;
				}
...
#endif
				send__connect_status(db, context, ONTICE_TYPE_OFFLINE, "read POLLERR | POLLNVAL | POLLHUP from current socket");
				do_disconnect(db, context, MOSQ_ERR_CONN_LOST);
				continue;
			}
		}
